...
void UNetReplaySubsystem::PlayNamedReplay(FString TargetReplay)
{
	TargetReplay == "" ? TargetReplay = ReplayName : TargetReplay = TargetReplay;
	AsyncTask(ENamedThreads::GameThread, [&, TargetReplay]()
		{
			if (UWorld* World = GetWorld())
			{
				if (UGameInstance* GameInstance = World->GetGameInstance())
...
{
	AsyncTask(ENamedThreads::GameThread, [&, DoPause]
		{
